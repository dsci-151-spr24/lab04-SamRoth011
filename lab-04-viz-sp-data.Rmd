...
***There are `r nrow(dennys)` rows and `r ncol(dennys)` columns. The variables are `r names(dennys)`. Each row is the data for one Dennys restaurant.***

2.  What are the dimensions of the La Quinta's dataset?
    What does each row in the dataset represent?
    What are the variables?
    
***There are `r nrow(laquinta)` rows and `r ncol(laquinta)` columns. The variables are `r names(laquinta)`. Each row is the data for one La Quinta's restaurant.***
...
***La Quinta hotels are mostly located in the Americas but can also be found in other parts of the world. Denny's are located in the US.***
...
4.  Now take a look at the data.
  ...
  ***The state variable could be used to see if a location is in the US or not.***
